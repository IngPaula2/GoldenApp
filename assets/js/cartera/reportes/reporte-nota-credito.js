/**
 * ====================================================================================
 * 📊 REPORTE NOTA CRÉDITO - GOLDEN APP
 * ====================================================================================
 * 
 * Este archivo genera el reporte de notas crédito que muestra las notas crédito
 * filtradas por ciudad y rango de fechas.
 * 
 * FUNCIONALIDADES:
 * - Busca notas crédito por ciudad y rango de fechas
 * - Muestra todas las notas crédito con sus detalles
...
(function(){
    ...
    function loadData(){
        try {
            const raw = localStorage.getItem('reporteNotaCreditoData');
            if (raw){
                const data = JSON.parse(raw);
                return data;
            }
    ...
    function processReportData() {
        ...
        try {
            ...
            const notasRaw = localStorage.getItem(`notasCredito_${data.cityCode}`);
    ...
    function renderReport() {
        ...
        if (pageRows.length === 0) {
            reportHTML += `
                        <tr>
                            <td colspan="12" style="text-align: center; padding: 20px; border: 1px solid #000;">
                                No se encontraron notas crédito en el rango de fechas seleccionado
                            </td>
                        </tr>
            `;
        } else {
        ...
        try {
            ...
            const fullExcelHTML = `
                <html>
                    <head>
                        <meta charset='utf-8'>
                        <title>Reporte Nota Crédito</title>
                        <style>
                            body { 
            ...
            a.href = url;
            a.download = `reporte-nota-credito-${new Date().getTime()}.xls`;
            a.click();
    ...
    window.exportDOC = function() {
        try {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `reporte-nota-credito-${new Date().getTime()}.doc`;
            a.click();
            URL.revokeObjectURL(url);
        } catch (error) {
